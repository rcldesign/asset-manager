 ...
 */
async function handleCreate(params: any, next: (params: any) => Promise<any>) {
  const result = await next(params);
  
  if (result && result.id) {
    // Create sync metadata for new entity
    await createSyncMetadata(params.model, result.id, params.args.data);
  }
 ...
 */
async function handleUpdate(params: any, next: (params: any) => Promise<any>) {
  ...
  if (entityIds.length > 0) {
    for (const entityId of entityIds) {
      await updateSyncMetadata(params.model, entityId, params.args.data);
    }
  }
 ...
 */
async function handleDelete(params: any, next: (params: any) => Promise<any>) {
 ...
 */
async function handleUpsert(params: any, next: (params: any) => Promise<any>) {
  const result = await next(params);
  
  if (result && result.id) {
    // Check if this was a create or update
    const existingMetadata = await getSyncMetadata(params.model, result.id);
    
    if (existingMetadata) {
      await updateSyncMetadata(params.model, result.id, params.args.create || params.args.update);
    } else {
      await createSyncMetadata(params.model, result.id, params.args.create || params.args.update);
    }
  }
 ...
 */
async function getEntityIds(params: any): Promise<string[]> {
  ...
  if (action === 'update' || action === 'delete') {
    ...
    ids.push(...entities.map((e: any) => e.id));
  }
 ...
 */
async function getSyncMetadata(entityType: string, entityId: string) {
  const { prisma } = await import('./prisma');
 ...
 */
async function createSyncMetadata(entityType: string, entityId: string, data: any) {
  const { prisma } = await import('./prisma');
 ...
 */
async function updateSyncMetadata(entityType: string, entityId: string, data: any) {
  const { prisma } = await import('./prisma');
 ...
 * Mark sync metadata as deleted
 */
async function markSyncMetadataDeleted(entityType: string, entityId: string) {
  const { prisma } = await import('./prisma');
...
function extractUserId(data: any): string | null {
  ...
  if (data.updatedByUserId) return data.updatedByUserId;
  if (data.createdByUserId) return data.createdByUserId;
